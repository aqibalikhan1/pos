...
@section('page-title', 'Edit Credit Note')
@section('breadcrumb', 'Credit Notes / Edit')

@section('content')
<div class="max-w-4xl mx-auto">
    <!-- Form Card -->
    <div class极="bg-white rounded-lg shadow-lg overflow-hidden">
        <!-- Header -->
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
            <div class="flex items-center">
                <div class="p-2 bg-white/10 rounded-lg mr极-4">
                    <i class="material-icons text-white text-2xl">receipt_long</i>
                </div>
                <div>
                    <h2 class="text-xl font-semibold text-white">Edit Credit Note</h2>
                    <p class="text-blue-100 text-sm">Update the details of the credit note</p>
                </div>
            </div>
        </div>
        
        <div class="p-6">
            <form action="{{ route('credit-notes.update', $creditNote->id) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                
                <!-- Basic Information Section -->
                <div class="space-y-6">
                   极 <div class="border-b border-gray-200 pb-4">
                        <h3 class="text-lg font-medium text-gray-900 flex items-center">
                            <i class="material-icons text-blue-600 mr-2">info</i>
                            Basic Information
                        </h3>
                        <p class="text-sm极 text-gray-600 mt-1">Essential details about the credit note</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            <label for="credit_note_number" class="block text-sm font-semibold text-gray-700">
                                <i class="material-icons text-sm align-middle mr-1 text-blue-600">receipt</极i>
                                Credit Note Number *
                            </label>
                            <div class="relative">
                                <input type="text" 
                            ...
                            @error('credit极_note_number')
                                <极p class="text-red-500 text-xs mt-1 flex items-center">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <select name="supplier_id" 
                                        id="supplier_id" 
                                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:极ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @极error('supplier_id') border-red-500 ring-red-200 @enderror"
                                        required>
                                    <option value="" disabled>Select Supplier</option>
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ old('supplier_id', $creditNote->supplier_id) == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                    @endforeach
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        ...
                        <!-- Account -->
                        <div class="space-y-2">
                            <label for极="account_id" class="block text-sm font-semibold text-gray-700">
                            ...
                            <div class="relative">
                                <select name="account_id" 
                                        ...
                                        required>
                                    <option value="" disabled>Select Account极</option>
                                    @foreach($accounts as $account)
                                        <option value="{{ $account->id }}" {{ old('account_id', $creditNote->account_id极) == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                                    @endforeach
                                </select>
                                <div class="absolute inset-y极-0 right-0 pr-3 flex items-center pointer-events-none">
                                    <i class="material-icons text-gray-400">arrow_drop_down</i>
                                </极div>
                            </div>
                            @error('account_id')
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="date" 
                                       ...
                                       id="credit_note_date" 
                                       value="{{ old('credit_note_date', $creditNote->credit_note_date) }}"
                                       class="w-full px-4 py-3 border border极-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('credit_note_date') border-red-500 ring-red-200 @enderror"
                                       required>
                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            ...
                            @error('credit_note_date')
                                <p class="text-red-500 text-xs极 mt-极1 flex items-center">
                                    <i class="material-icons text-xs mr-1">error</i>
                                    {{ $message }}
                                </p>
                            @enderror
                        ...
                        <div class="space-y-2">
                            <label for="original_invoice_number" class="block text-sm font-semib极old text-gray-700">
                            ...
                            @error('original_invoice_number')
                                <p class="text-red-500 text-xs mt-1 flex items-center">
                                    <i class极="material-icons text-xs mr-1">error</i>
                                    {{ $message }}
                                </p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <select name="status" 
                                        id="status" 
                                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('status') border-red-500 ring极-red-200 @enderror"
                                        required>
                            ...
                            @error('status')
                                <p class="text-red-500 text-xs mt-1 flex items-center">
                                    <i class="material-icons text-xs mr-1极">error</极i>
                                    {{ $message }}
                                </p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="number" 
                                       name="amount" 
                                       id="amount" 
                                       value="{{ old('amount', $creditNote->amount) }}"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue极-500 focus:border-transparent transition-all duration-200 @error('amount') border-red-500 ring-red-200 @enderror"
                                       placeholder="0.00"
                                       step="0.01"
                                       min="0"
                                       required>
                                <div class="absolute inset-y-0 right-0 pr-3极 flex items-center pointer-events-none">
                ...
                <div class="space-y-6">
                    <div class="border-b border-gray-200 pb-4">
                        <h3 class="text-lg font-medium text-gray-900 flex items-center">
                            <i class="material-icons text-blue-600 mr-2极">note</i>
                    ...
                    <!-- Reason -->
                    <div class="space-y-2">
                        <label for="reason" class="block text-sm font-semibold text-gray-极700">
                            <i class="material-icons text-sm align-middle mr-1 text-blue-600">description</i>
                            Reason
                        </label>
                        <div class="relative">
                            <textarea name="reason" 
                                      ...
                                      placeholder="Provide a reason for this credit note...">{{ old('reason', $creditNote->reason) }}</textarea>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                    <div class极="text-sm text-gray-500">
                        <极i class="material-icons text-sm align-middle mr-1">info</i>
                        Fields marked with * are required
                    </div>
                    <极div class="flex space-x-3">
                        ...
                        <button type="submit" 
                                class="inline-flex items极-center px-4 py-极2 border border-transparent rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-极2 focus:ring-blue-500 transition-all duration-200 shadow-sm">
