...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Edit Supplier Payment</h1>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('accounts.supplier-payments.update', $supplierPayment) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="payment_number" id="payment_number" 
                               ...
                               value="{{ old('payment_number', $supplierPayment->payment_number) }}" required>
                    ...
                    <div>
                        ...
                        <select name="supplier_id" id="supplier_id" 
                            ...
                            @foreach($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" {{ old('supplier_id', $supplierPayment->supplier_id) == $supplier->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="purchase_id" id="purchase_id" 
                            ...
                            @foreach($purchases as $purchase)
                                <option value="{{ $purchase->id }}" {{ old('purchase_id', $supplierPayment->purchase_id) == $purchase->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="payment_date" id="payment_date" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                               value="{{ old('payment_date', $supplierPayment->payment_date->format('Y-m-d')) }}" required>
                        @error('payment_date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" step="0.01" min="0"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                               value="{{ old('amount', $supplierPayment->amount) }}" required>
                        @error('amount')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="payment_method" id="payment_method" 
                            ...
                            <option value="credit_card" {{ old('payment_method', $supplierPayment->payment_method) == 'credit_card' ? 'selected' : '' }}>Credit Card</option>
                        </select>
                ...
                <div class="mt-6">
                    ...
                    <input type="text" name="reference_number" id="reference_number" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                           value="{{ old('reference_number', $supplierPayment->reference_number) }}">
                ...
                <div class="mt-6">
                    ...
                    <textarea name="notes" id="notes" rows="3" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $supplierPayment->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
