...
@section('breadcrumb', 'Tax Rates / Edit')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="stats-card">
            ...
            <div class="stats-card-title">Current Rate</div>
            <div class="stats-card-value">{{ $taxRate->rate }}%</div>
        </div>
        ...
        <div class="stats-card">
            ...
            <div class="stats-card-title">Tax Type</div>
            <div class="stats-card-value">{{ $taxRate->taxType->name }}</div>
        </div>
        ...
        <div class="stats-card">
            ...
            <div class="stats-card-title">Effective Since</div>
            <div class="stats-card-value">{{ $taxRate->effective_date->format('M d, Y') }}</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
            <div class="flex items-center">
                ...
                <div>
                    <h2 class="text-xl font-semibold text-white">Edit Tax Rate</h2>
                    <p class="text-blue-100 text-sm">Update tax rate configuration for your products and services</p>
                </div>
            </div>
        </div>
        
        <div class="p-6">
            <form action="{{ route('settings.tax-rates.update', $taxRate->id) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                            ...
                            <div class="relative">
                                <select name="tax_type_id" 
                                    ...
                                    @foreach($taxTypes as $taxType)
                                        <option value="{{ $taxType->id }}" {{ old('tax_type_id', $taxRate->tax_type_id) == $taxType->id ? 'selected' : '' }}>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Fixed Amount -->
                        <div class="space-y-2">
                            <label for="fixed_amount" class="block text-sm font-semibold text-gray-700">
                            ...
                            <div class="relative">
                                <input type="number" 
                                       name="fixed_amount" 
                                       id="fixed_amount" 
                                       value="{{ old('fixed_amount', $taxRate->fixed_amount) }}"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('fixed_amount') border-red-500 ring-red-200 @enderror"
                                       placeholder="0.00"
                                       step="0.01"
                                       min="0">
                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="date" 
                                       name="effective_date" 
                                       id="effective_date" 
                                       value="{{ old('effective_date', $taxRate->effective_date->format('Y-m-d')) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="date" 
                                       name="end_date" 
                                       id="end_date" 
                                       value="{{ old('end_date', $taxRate->end_date ? $taxRate->end_date->format('Y-m-d') : '') }}"
