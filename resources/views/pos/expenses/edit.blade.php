    ...
    <div class="md-card">
        <div class="md-card-header">
            <div class="flex justify-between items-center">
                <h2 class="md-card-title">Edit Expense</h2>
        ...
        <form action="{{ route('expenses.update', $expense) }}" method="POST" class="md-card-content">
            @csrf
            @method('PUT')
            ...
            <div class="mt-4" id="recurring_frequency_div" style="display: {{ $expense->is_recurring ? 'block' : 'none' }};">
                <label for="recurring_frequency" class="block text-sm font-medium text-gray-700">
                    Frequency
                </label>
                <select name="recurring_frequency" id="recurring_frequency" 
                    ...
                    <option value="daily" {{ old('recurring_frequency', $expense->recurring_frequency) == 'daily' ? 'selected' : '' }}>Daily</option>
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" id="notes" rows="3" 
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm @error('notes') border-red-500 @enderror">{{ old('notes', $expense->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
