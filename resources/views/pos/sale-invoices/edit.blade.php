...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Sale Invoice #{{ $saleInvoice->invoice_number }}</h1>
        <div class="flex space-x-2">
            <a href="{{ route('sale-invoices.show', $saleInvoice) }}" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
                Cancel
            </a>
        </div>
    </div>

    <form id="saleInvoiceForm" action="{{ route('sale-invoices.update', $saleInvoice) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="employee_id" id="employee_id" required 
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ $saleInvoice->employee_id == $employee->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="sale_date" id="sale_date" value="{{ old('sale_date', $saleInvoice->sale_date->format('Y-m-d')) }}" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('sale_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="sale_type" id="sale_type" required 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="cash" {{ $saleInvoice->sale_type == 'cash' ? 'selected' : '' }}>Cash</option>
                        <option value="credit" {{ $saleInvoice->sale_type == 'credit' ? 'selected' : '' }}>Credit</option>
                    </select>
                    @error('sale_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method" required 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="cash" {{ $saleInvoice->payment_method == 'cash' ? 'selected' : '' }}>Cash</option>
                        <option value="credit" {{ $saleInvoice->payment_method == 'credit' ? 'selected' : '' }}>Credit</option>
                        <option value="bank_transfer" {{ $saleInvoice->payment_method == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
                        <option value="cheque" {{ $saleInvoice->payment_method == 'cheque' ? 'selected' : '' }}>Cheque</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $saleInvoice->due_date?->format('Y-m-d')) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('due_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-4">
                ...
                <textarea name="notes" id="notes" rows="3" 
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                          placeholder="Any additional notes...">{{ old('notes', $saleInvoice->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
                    ...
                    <tbody id="itemsTableBody" class="bg-white divide-y divide-gray-200">
                        @foreach($saleInvoice->items as $index => $item)
                            <tr class="item-row">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <input type="number" name="items[{{ $index }}][tax_amount]" 
                                           value="{{ old('items.' . $index . '.tax_amount', $item->tax_amount) }}" 
                                           min="0" step="0.01"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="paid_amount" id="paid_amount" 
                           value="{{ old('paid_amount', $saleInvoice->paid_amount) }}" 
                           min="0" step="0.01" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('paid_amount')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mt-6 border-t pt-4">
                <div class="space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span id="subtotal">${{ number_format($saleInvoice->subtotal, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm font-medium text-gray-700">Total Discount:</span>
                        <span id="totalDiscount">${{ number_format($saleInvoice->discount_amount, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm font-medium text-gray-700">Total Tax:</span>
                        <span id="totalTax">${{ number_format($saleInvoice->tax_amount, 2) }}</span>
                    </div>
                    <hr class="my-2">
                    <div class="flex justify-between font-semibold">
                        <span class="text-sm font-medium text-gray-700">Total Amount:</span>
                        <span id="totalAmount">${{ number_format($saleInvoice->total_amount, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm font-medium text-gray-700">Due Amount:</span>
                        <span id="dueAmount">${{ number_format($saleInvoice->due_amount, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
